...
# Network 神经网络对象，提供API接口。它由若干 层对象 组成以及 连接对象 组成。
# Layer 层对象，由多个节点组成。
# Node 节点对象计算和记录节点自身的信息(比如输出值a、误差项等)，以及与这个节点相关的上下游的连接。
# Connection 每个 连接对象 都要记录该连接的权重。
# Connections 仅仅作为Connection的集合对象，提供一些集合操作。
...
def train_data_set():
    ...
    for i in range(0, 256, 8):
        ...
        data_set.append(n)
        labels.append(n)
    return list(labels), list(data_set)
